...
camera.resolution = (736, 400) # Setting camera resolution (lower for speed)
...
while(True):
    masked = isolateLED(camera) # mask the image to isolate the red and green 
    # cv2.imshow('Masked Image', masked) # Displays the masked image at every iteration
    # cv2.waitKey(0)
    color = detColor(masked) # Determine the color LED (if any)
